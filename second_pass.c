...

{
	...
	printf("direction type: %d\n" , (*ld_arr)[i].dir->d_type);
		...
		printf("data count: %d\n" , data_count);
			...
			printf("curr_index binary table: %d\n" , *instruction_binary_table->curr_index);

          intToTwosComplement((*ld_arr)[i].dir->d_content->d_arr->data_arr[j] , instruction_binary_table->lines_as_binary[*instruction_binary_table->curr_index]);
